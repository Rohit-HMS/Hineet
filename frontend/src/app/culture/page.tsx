...
import { fetchJobs, fetchCulturePage, type CulturePageData } from "@/lib/api";

const instrumentSans = Instrument_Sans({ subsets: ["latin"] });

const defaultCultureData: CulturePageData = {
  badge: "Who We Are",
  heading: "Our *Culture*",
...
export default function Culture() {
  ...
  const [initialData, setInitialData] = React.useState<CulturePageData | undefined>(undefined);

  React.useEffect(() => {
    fetchJobs().then(jobs => setJobsLength(jobs.length));
    fetchCulturePage().then((data) => {
      if (data) {
        setInitialData(data);
      }
    });
  ...
  const { data: liveData } = useLivePreview<CulturePageData>({
  ...
  const activeData = resolvedData || defaultCultureData;
  ...
  const valuesList = activeData.values && activeData.values.length > 0 ? activeData.values : defaultCultureData.values || [];
    ...
    <div className="pt-40 pb-20 w-full min-h-screen relative z-10 bg-transparent">
      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-32 relative z-10">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-br from-[#2563EB]/05 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-250" />
            <div className="absolute -top-20 -right-20 w-64 h-64 bg-[#2563EB]/08 rounded-full blur-[80px] group-hover:bg-[#2563EB]/16 transition-opacity duration-250" />
            <h3 className={`${instrumentSans.className} text-[#FFFFFF] text-4xl md:text-[4rem] font-light tracking-wide leading-[1.1] mb-6 drop-shadow-sm relative z-10`}>
              {activeData.bentoWideTitle}
            </h3>
            <p className="text-[#737373] text-xl relative z-10 max-w-2xl leading-relaxed">
              {activeData.bentoWideDescription}
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-gradient-to-br from-[#2563EB] to-[#1D4ED8] text-white p-12 rounded-[2.5rem] flex flex-col justify-between group hover:scale-[1.02] hover:shadow-[0_20px_60px_rgba(37,99,235,0.4)] transition-all duration-200 min-h-[400px]"
          >
            <h3 className={`${instrumentSans.className} text-[#FFFFFF] text-4xl md:text-[4rem] font-light tracking-wide leading-[1.1] mb-6 drop-shadow-sm`}>
              {activeData.bentoAccentTitle}
            </h3>
            <div className="flex justify-between items-end">
              <span className="text-8xl font-black tracking-tighter text-white/90">{jobsLength}</span>
              <Link href="/careers">
          ...
          {valuesList.map((val, i) => (
            ...
            >
              ...
              <h4 className="text-2xl font-bold tracking-tighter mb-3 text-[#FFFFFF] group-hover:text-[#2563EB] transition-colors">
                {val.title}
              </h4>
