...
import { fetchNavbar } from "@/lib/api";
...
export default function Navbar() {
  ...
  const [initialNavbar, setInitialNavbar] = useState<{ links: typeof defaultNavLinks } | undefined>(undefined);
  const pathname = usePathname();
  const { theme } = useTheme();
  const isDarkTheme = theme === "dark";

  useEffect(() => {
    fetchNavbar().then((data) => {
      if (data && data.links && data.links.length > 0) {
        setInitialNavbar(data);
        setNavLinks(data.links);
      }
    });
  }, []);

  const { data: liveNavbar } = useLivePreview({
    initialData: initialNavbar as any,
    serverURL: process.env.NEXT_PUBLIC_PAYLOAD_URL || "http://localhost:4000",
    depth: 1,
  });

  const activeLinks = (liveNavbar?.links && liveNavbar.links.length > 0) ? liveNavbar.links : navLinks;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      setHiddenOnScroll(currentScrollY > 80 && currentScrollY > lastScrollY);
  ...
  return (
    <>
      <header
        className={cn(
          "fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out transform-gpu",
          hiddenOnScroll ? "-translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100",
          scrolled
            ? isDarkTheme
              ? "py-3 bg-black border-b border-white/10 shadow-sm"
      ...
      >
        <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
          ...
          <Link href="/" className="relative z-50 flex items-center group">
            ...
            >
              ...
              <div className="flex flex-col justify-center leading-none mt-0.5">
                <span className="text-[1.15rem] font-bold tracking-tight text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] transition-colors duration-300">
                  HINEET TECH
                </span>
                <span className="text-[0.6rem] font-semibold tracking-[0.2em] text-[var(--text-muted)] mt-1 uppercase">
                  PRIVATE LIMITED
                </span>
              </div>
            </motion.div>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {activeLinks.filter((link: any) => link.name && link.href).map((link: any, index: number) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive ? "text-[var(--accent-primary)]" : "text-[var(--text-primary)] hover:text-[var(--accent-primary)]"
                  )}
                >
                  {link.name}
                  <span
          ...
          >
            ...
            <div className="mb-8 flex flex-col items-center text-center">
              ...
              <div className="flex flex-col items-center leading-none">
                ...
                <span className="text-xs font-semibold tracking-[0.2em] text-[var(--text-muted)] mt-2 uppercase">
                  PRIVATE LIMITED
                </span>
              </div>
            </div>

             {activeLinks.filter((link: any) => link.name && link.href).map((link: any, i: number) => (
              <motion.div
                key={link.id || link.name || i}
